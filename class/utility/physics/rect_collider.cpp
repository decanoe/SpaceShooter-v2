...
void RectCollider::draw() const {
    ...
    SDL_Rect rect = this->bounding_box().offset(Graphics::world_to_screen(this->transform->position)).to_sdl_rect();
    SDL_RenderDrawRect(Graphics::screen.get_renderer(), &rect);
}
